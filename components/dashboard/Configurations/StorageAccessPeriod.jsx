...
import { DeleteTypeModal, DeleteConfigModal, AddStorageTypeModal, AddAccessPeriodModal } from "../../../components";
import { getType, getAccessPeriod } from "../../../redux/features/configurations.slice";

const StorageAccessPeriod = () => {
  const { accessperiod } = useSelector((state) => state.configuration);
  const [editVal, setEditVal] = useState([]);
  const [Id, setId] = useState("");
  const edit = (_id) => {
    const val = accessperiod.filter((val) => _id === val._id);
    setEditVal(val);
  };

  return (
    <Accordion title="Storage Access Period">
      ...
      <div>
        {accessperiod?.length === 0 ? (
          <div className=" text-center font-semibold text-sm md:text-base">No storage access period at this time</div>
        ) : (
          accessperiod?.map(({ label, _id }, indx) => (
            <div
              key={indx}
      ...
      <AddAccessPeriodModal details={editVal} />
      <DeleteConfigModal id={Id} config="storage-access-period" refresh={getAccessPeriod} />
    </Accordion>
  );
};
