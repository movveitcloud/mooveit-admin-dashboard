...
import { DeleteTypeModal, DeleteConfigModal, AddStorageTypeModal, AddNoticePeriodModal } from "../../../components";
import { getType, getAccessPeriod, getNoticePeriod } from "../../../redux/features/configurations.slice";

const StorageNoticePeriod = () => {
    ...
    <Accordion title="Storage Notice Period">
      ...
      <div>
        {noticeperiod?.length === 0 ? (
          <div className=" text-center font-semibold text-sm md:text-base">No storage notice period at this time</div>
        ) : (
          noticeperiod?.map(({ label, _id }, indx) => (
            <div
              key={indx}
      ...
      <AddNoticePeriodModal details={editVal} />
      <DeleteConfigModal id={Id} config="notice-period" refresh={getNoticePeriod} />
    </Accordion>
  );
};

export default StorageNoticePeriod;
