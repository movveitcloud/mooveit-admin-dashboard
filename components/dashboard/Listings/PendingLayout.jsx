...
const PendingLayout = ({ content, name }) => {
      ...
      <div className="overflow-auto rounded-lg">
        <table className="table table-compact w-full p-4 mb-8 ">
          ...
          <tbody className="w-full   ">
            {content &&
              content.map(({ Listing, Location, WithMoving, WithPacking, Approved, Deny, id }, index) => (
                <tr className="capitalize cursor-pointer border border-[#DCDCFF]   " key={index}>
                  ...
                  <td className="w-[15%] p-4 ">{Location}</td>

                  <td className="w-[15%] p-4">{WithMoving}</td>

                  <td className="w-[10%] p-4 ">{WithPacking}</td>
