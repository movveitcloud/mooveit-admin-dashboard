...
import { DashboardLayout, DownloadCSV, ListingsLayout, PaymentLayout, Tabs } from "../../components";
...
const Listings = () => {
  ...
  const tabItems = [
    ...
    { name: "Pending", count: pendingPayments.length },
    { name: "Refunded", count: refundedPayments.length },
  ];

  const listingCounts = [approvedPayments.length, pendingPayments.length, refundedPayments.length];
  const listingStatus = ["approved", "pending", "refunded"];
  ...
  const listingData = [approvedPayments, pendingPayments, refundedPayments];
  const csvFilename = ["MovveIt_Approved_Payments", "MovveIt_Pending_Payments", "MovveIt_Refunded_Payments"];
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
          ...
          <div className="bg-white p-8  shadow ">
            ...
            {listingCounts[activeTab] === 0 ? (
              ...
              <PaymentLayout listingStatus={listingStatus[activeTab]} />
            )}
          </div>
        </motion.div>
      )}
